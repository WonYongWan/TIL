...
- [2월24일](#2월24일)
  ...
  - [Node.js 패키지 생성 및 외부 패키지 사용하기](#nodejs-패키지-생성-및-외부-패키지-사용하기)
...
- 압축을 하면 용량이 거의 반으로 줄어드는 경우가 많다. Content-Encoding을 기입해야 한다.
...
명령어를 입력해서 프로그램을 실행하는 것을 CLI라고 한다. (터미널)
...
```
<!-- 패키지 생성 -->
빈폴더 생성 ex) udemy-react-package-example
npm init
패키지명 입력 ex) package-example2
버전명 입력 ex) 처음이면 1.0.0
설명 입력(입력 안해도됨.)
entry point => default(index.js) 여러개의 파일중에 실행시킬 파일(main)
text command 넘김
git repository 넘김
keywords 넘김
author(본인 닉네임 또는 넘김)
...
```
<!-- 패키지 생성을 하면 package.json이 생성됨 -->
"scripts" = 패키지를 개발하면서 자주 실행해야 되는 명령어를 사전에 정의

ex) npm start
"start": "node index.js" => 터미널에 start만 입력해도 node index.js명령어가 실행된다.
```
외부 패키지 사용

[npm](https://www.npmjs.com/)접속 -> 필요한 패키지 검색 ex) 랜덤한 컬러 -> randomcolor
```
Install영역 명령어 복사 및 터미널 입력 ex) npm i randomcolor

설치하면 package.json에 "dependencies"와 설치한 패키지 버전이 생김
만약 패키지 버전에 ^0.6.2 같은 식으로 ^가 있으면 패키지의 정확한 버전을 명시한 것이 아니라 0.6.2 이상으로만 설치가 된다는 뜻이다.
```

package-lock.json은 우리 패키지에 설치된 외부패키지들의 설치된 정확한 버전을 기록한다.

```js
// 외부에서 가져온 패키지들은 경로를 명시하지 않아도 된다.
